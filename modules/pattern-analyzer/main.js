module.exports = (function (root) {
  ...
  function createPopupHtml() {
    return [
      ...
      "html,body{margin:0;padding:0;background:#f5f7fa;color:#222;font-family:'Malgun Gothic','Apple SD Gothic Neo',sans-serif}body{padding:20px;box-sizing:border-box}",
      ".header{display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;padding-bottom:10px;border-bottom:2px solid #3f51b5}.header h2{margin:0;color:#3f51b5}",
      ".card{background:#fff;border:1px solid #dde3ea;border-radius:8px;box-shadow:0 4px 12px rgba(15,23,42,.06)}.controls,.filters,.actions{display:flex;gap:12px;flex-wrap:wrap;align-items:center;padding:14px 16px;margin-bottom:16px}",
      "label{font-weight:700;color:#475569}input,select,textarea{border:1px solid #cfd8e3;border-radius:6px;box-sizing:border-box;font:inherit}input[type='date'],input[type='number'],input[type='text']{height:36px;padding:0 10px}",
      "button{height:36px;padding:0 14px;border:1px solid #2f44c0;background:#3f51b5;color:#fff;border-radius:6px;cursor:pointer;font-weight:700}button.secondary{background:#fff;color:#3f51b5}button.green{background:#2e7d32;border-color:#2e7d32}button.gray{background:#616161;border-color:#616161}button.danger{background:#d32f2f;border-color:#d32f2f}button:disabled{background:#c9cfdb;border-color:#c9cfdb;cursor:not-allowed}",
      ".tabs{display:flex;gap:6px;margin-bottom:16px}.tab{height:38px;padding:0 18px;border:1px solid #cfd8e3;border-radius:8px 8px 0 0;background:#eef2ff;color:#334155;cursor:pointer;font-weight:700}.tab.active{background:#3f51b5;color:#fff;border-color:#3f51b5}",
      ".content{display:none}.content.active{display:block}.message{padding:12px 14px;border-radius:8px;margin-bottom:16px;font-weight:700}.message.error{background:#ffebee;color:#b71c1c}.message.success{background:#e8f5e9;color:#1b5e20}",
      ".info{font-size:12px;color:#64748b;background:#f8fafc;border-radius:999px;padding:5px 10px}.summary{display:none;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;margin-bottom:16px;padding:14px 16px}",
      ".summary-item{background:#fff;border:1px solid #dde3ea;border-radius:8px;padding:10px 12px}.summary-item .label{display:block;font-size:12px;color:#64748b;margin-bottom:4px}.summary-item .value{font-size:16px;font-weight:700;color:#1e293b}.tags{display:flex;gap:6px;flex-wrap:wrap}.tag{display:inline-flex;align-items:center;padding:2px 8px;border-radius:999px;background:#e8eaf6;color:#3f51b5;font-size:12px}",
      "table{width:100%;border-collapse:collapse;background:#fff}th,td{border:1px solid #e5e7eb;padding:8px 10px;text-align:center;font-size:13px;vertical-align:middle}th{background:#f8fafc;position:sticky;top:0;z-index:1}td.left{text-align:left}tr.clickable{cursor:pointer}tr:nth-child(even){background:#fbfcfe}tr.leftover{background:#ffebee}tfoot tr{background:#f1f5f9;font-weight:700}",
      ".badge{display:inline-flex;align-items:center;gap:4px;padding:3px 8px;border-radius:999px;font-size:12px;font-weight:700;margin:2px}.badge.complete{background:#e8f5e9;color:#1b5e20}.badge.pending{background:#fff8e1;color:#ef6c00}",
      ".link{color:#334ec8;text-decoration:underline;cursor:pointer;font-weight:700}.multi{position:relative;min-width:220px}.multi-selected{height:36px;padding:0 12px;border:1px solid #cfd8e3;border-radius:6px;background:#fff;display:flex;align-items:center;justify-content:space-between;cursor:pointer}.multi-badges{display:flex;gap:6px;flex-wrap:wrap;margin-top:6px}.multi-badge{display:inline-flex;align-items:center;background:#eef2ff;border-radius:999px;padding:2px 8px;font-size:12px}.multi-badge-remove{margin-left:4px;cursor:pointer;font-weight:700}.multi-dropdown{display:none;position:absolute;left:0;right:0;top:100%;margin-top:4px;background:#fff;border:1px solid #cfd8e3;border-radius:8px;box-shadow:0 12px 30px rgba(15,23,42,.12);z-index:10;max-height:260px;overflow:auto}.multi-dropdown.show{display:block}.multi-search{padding:8px;border-bottom:1px solid #e5e7eb;background:#fff;position:sticky;top:0}.multi-search input{width:100%;height:34px}.multi-option{display:flex;gap:8px;align-items:center;padding:8px 10px;border-bottom:1px solid #f1f5f9}.multi-controls{display:flex;justify-content:space-between;align-items:center;padding:8px 10px;background:#f8fafc;position:sticky;bottom:0}.multi-actions{display:flex;gap:6px}.multi-action{height:28px;padding:0 8px;border:1px solid #cfd8e3;background:#fff;border-radius:6px;cursor:pointer}",
      ".overlay{display:none;position:fixed;inset:0;background:rgba(15,23,42,.45);align-items:center;justify-content:center;z-index:1000}.modal{width:min(680px,92vw);max-height:85vh;overflow:hidden;background:#fff;border-radius:12px;box-shadow:0 18px 60px rgba(15,23,42,.25);display:flex;flex-direction:column}.invoice-modal{width:min(560px,92vw)}.modal-head,.modal-foot{display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid #e5e7eb}.modal-foot{border-top:1px solid #e5e7eb;border-bottom:none;gap:10px;justify-content:flex-end}.modal-body{padding:18px;overflow:auto}.close{border:none;background:none;font-size:22px;cursor:pointer;color:#64748b}",
      ".stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-bottom:14px}.stat{background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:10px;text-align:center}.stat .label{font-size:12px;color:#64748b}.stat .value{font-size:20px;font-weight:700;color:#1e293b}.stat .value.click{cursor:pointer;color:#334ec8}.progress{height:18px;background:#edf2f7;border-radius:999px;overflow:hidden;margin-bottom:14px}.progress > div{height:100%;width:0;background:#2e7d32;transition:width .2s ease}.log{max-height:220px;overflow:auto;background:#0f172a;color:#e2e8f0;border-radius:8px;padding:10px;font-family:Consolas,'Courier New',monospace;font-size:12px}.log-item{padding:4px 0;border-bottom:1px solid rgba(255,255,255,.08)}.log-item:last-child{border-bottom:none}.log-item.success{color:#86efac}.log-item.error{color:#fca5a5}",
      ".list{max-height:260px;overflow:auto;border:1px solid #e5e7eb;border-radius:8px;margin-bottom:12px}.list-item{padding:7px 10px;border-bottom:1px solid #f1f5f9;font-family:Consolas,'Courier New',monospace}.list-item:nth-child(odd){background:#f8fafc}.list-item:last-child{border-bottom:none}.textarea{width:100%;min-height:110px;padding:10px;border:1px solid #cfd8e3;border-radius:8px;resize:vertical;margin-bottom:12px;font-family:Consolas,'Courier New',monospace}.success{display:none;margin-bottom:12px;padding:8px 10px;border-radius:8px;background:#e8f5e9;color:#1b5e20;font-weight:700}",
      "#print-section{display:none}@media print{body{padding:0;background:#fff}.header,.controls,.tabs,.filters,.actions,.overlay{display:none!important}#print-section{display:block!important}}</style></head><body>",
      "<div class='header'><h2>" + MODULE_NAME + "</h2></div><div id='message-root'></div>",
      "<div class='card controls'><label for='date-input'>출력일</label><input type='date' id='date-input'><div><label>판매처</label><div id='site-multiselect' class='multi'><div class='multi-selected'><span class='multi-text'>전체</span><span>▼</span></div><div class='multi-badges'></div></div></div><div><label>택배사</label><div id='expr-multiselect' class='multi'><div class='multi-selected'><span class='multi-text'>전체</span><span>▼</span></div><div class='multi-badges'></div></div></div><button id='search-button'>조회</button><span id='data-info' class='info'>데이터 정보: 없음</span></div>",
      ...
      "<div id='pattern-content' class='content'><div class='card filters'><label>포함검색어 <input type='text' id='include-input' placeholder='예: 김치, 사과'></label><label>제외검색어 <input type='text' id='exclude-input' placeholder='예: 김치, 사과'></label><label>최소반복수 <input type='number' id='min-rep-input' min='1' value='1' style='width:88px'></label><button id='print-pattern-button' class='gray'>인쇄</button></div><div id='pattern-summary' class='summary'></div><div class='card' style='overflow:auto'><table><thead><tr><th>순번</th><th>제품명</th><th>관리명</th><th>옵션명</th><th>수량</th><th>반복수</th><th>배송상태</th><th>송장번호</th><th><input type='checkbox' id='select-all-patterns'></th></tr></thead><tbody id='pattern-table-body'><tr><td colspan='9'>패턴 데이터가 없습니다.</td></tr></tbody><tfoot id='pattern-table-foot'></tfoot></table></div><div class='card actions'><button id='show-invoices-button' class='secondary'>종합송장</button><button id='shipping-button' class='green'>종합출고</button></div></div>",
      "<div id='print-section'></div>",
      "<div id='shipping-modal' class='overlay'><div class='modal'><div class='modal-head'><h3 id='shipping-title'>종합 출고 처리</h3><button id='close-shipping-modal' class='close'>&times;</button></div><div class='modal-body'><div class='stats'><div class='stat'><div class='label'>총 건수</div><div id='total-invoices' class='value'>0</div></div><div class='stat'><div class='label'>출고완료</div><div id='completed-invoices' class='value click'>0</div></div><div class='stat'><div class='label'>출고실패</div><div id='failed-invoices' class='value click'>0</div></div><div class='stat'><div class='label'>진행률</div><div id='progress-percentage' class='value'>0%</div></div></div><div class='progress'><div id='shipping-progress-bar'></div></div><div style='margin-bottom:12px'><label><input type='checkbox' id='cancel-mode'> 출고취소 모드</label></div><div style='margin-bottom:12px'><label for='shipping-date'>출고날짜</label><input type='date' id='shipping-date' style='width:100%'></div><div style='margin-bottom:12px'><label for='parallel-count'>병렬 처리 개수</label><div style='display:flex;gap:8px;align-items:center'><button id='decrease-parallel' class='secondary'>-</button><input type='number' id='parallel-count' min='1' max='10' value='2' style='width:64px;text-align:center'><button id='increase-parallel' class='secondary'>+</button><small>1~10 범위 내(권장: 2)</small></div></div><div id='shipping-log' class='log'></div></div><div class='modal-foot'><button id='start-shipping-button' class='green'>작업시작</button><button id='stop-shipping-button' class='danger' style='display:none'>작업중지</button><button id='close-shipping-button' class='secondary'>닫기</button></div></div></div>",
      "<div id='invoice-modal' class='overlay'><div class='modal invoice-modal'><div class='modal-head'><h3 id='invoice-title'>송장번호 목록</h3><button id='close-invoice-modal' class='close'>&times;</button></div><div class='modal-body'><div style='margin-bottom:10px'>총 <span id='invoice-count'>0</span>개의 송장번호가 있습니다.</div><div id='invoice-copy-success' class='success'>클립보드에 복사되었습니다.</div><div id='invoice-list' class='list'></div><textarea id='invoice-textarea' class='textarea' readonly></textarea><div style='display:flex;gap:8px;flex-wrap:wrap'><button id='copy-newline-button'>개행으로 복사</button><button id='copy-comma-button'>쉼표로 복사</button><button id='download-csv-button' class='secondary'>CSV 다운로드</button></div></div></div></div></body></html>",
    ].join("");
  }
  ...
  function displayPatterns(popupState) {
    ...
    popupState.renderedPatterns.forEach((pattern, index) => {
      ...
      pattern.items.forEach((item, itemIndex) => {
        const row = doc.createElement("tr");
        if (leftover) row.className = "leftover";
        if (itemIndex === 0) {
  ...
  function updateShippingLabels(popupState, isCancelMode) {
    ...
    doc.getElementById("shipping-progress-bar").style.backgroundColor = isCancelMode ? "#d32f2f" : "#2e7d32";
  }
