(function (root, factory) {
  ...
  function buildModuleUiCss() {
    return [
      ...
      ".tm-ui-root .tm-ui-scroll .tm-ui-table thead th,.tm-ui-root .tm-ui-scroll table thead th{position:sticky;top:0;z-index:3;background:#f0f2f4!important;box-shadow:inset 0 -1px 0 var(--tm-border)}",
      ...
      ".tm-ui-dock{--tm-dock-surface:" + TOKENS.surface + ";--tm-dock-surface-alt:" + TOKENS.surfaceAlt + ";--tm-dock-primary:" + TOKENS.primary + ";--tm-dock-primary-strong:" + TOKENS.primaryStrong + ";--tm-dock-text:" + TOKENS.text + ";--tm-dock-muted:" + TOKENS.muted + ";--tm-dock-border:" + TOKENS.border + ";--tm-dock-shadow:" + TOKENS.shadow + ";position:fixed;top:12px;right:12px;z-index:9999;display:grid;justify-items:end;gap:10px;pointer-events:none}",
      ".tm-ui-dock>*{pointer-events:auto}",
      ".tm-ui-dock.is-open{z-index:10000}",
      ".tm-ui-dock__toggle{display:inline-flex;align-items:center;gap:8px;min-height:36px;padding:0 14px;border:1px solid var(--tm-dock-border);border-radius:14px;background:rgba(255,255,255,.98);color:var(--tm-dock-text);box-shadow:0 14px 28px rgba(17,25,32,.12);transition:background-color .16s ease,border-color .16s ease,box-shadow .16s ease,transform .16s ease;text-decoration:none}",
      ".tm-ui-dock__toggle:hover{background:var(--tm-dock-surface-alt);transform:translateY(-1px)}",
      ".tm-ui-dock__toggle.is-open{background:var(--tm-dock-surface);border-color:#c7d1dc;box-shadow:0 18px 34px rgba(17,25,32,.16)}",
      ...
      ".tm-ui-dock__panel{position:relative;display:none;max-height:calc(90vh - 46px);overflow:auto;resize:both;border:1px solid var(--tm-dock-border);border-radius:20px;background:rgba(255,255,255,.99);box-shadow:0 28px 56px rgba(17,25,32,.18),0 8px 20px rgba(17,25,32,.08);backdrop-filter:none}",
      ".tm-ui-dock__panel.is-open{display:block}",
      ...
      "@media (max-width:768px){.tm-ui-root .tm-ui-panel-head{padding:16px}.tm-ui-root .tm-ui-toolbar{padding:12px}.tm-ui-root .tm-ui-kpi__value{font-size:18px}.tm-ui-root .tm-ui-section-head{align-items:flex-start}.tm-ui-dock{top:8px;right:8px}}",
    ].join("");
  }
