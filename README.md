...
GitHub 레지스트리 기반 Tampermonkey 로더와 원격 스크립트 저장소다. Tampermonkey에는 `loader/loader.user.js`만 설치하고, 실제 스크립트는 GitHub raw에서 레지스트리와 버전을 확인해 내려받는다.
...
1. Tampermonkey에 `loader/loader.user.js`를 등록한다.
2. 현재 페이지에서 로더 메뉴를 열어 스크립트 상태를 확인한다.
3. 필요하면 `현재 페이지 스크립트 동기화`를 실행한다.
