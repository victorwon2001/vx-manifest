...
test("inbound data viewer table rows keep every column centered", () => {
  ...
  assert.match(html, /data-tm-align="center">팬덤코리아/);
  assert.match(html, /data-tm-align="center">테스트관리명/);
  assert.match(html, /data-tm-align="center">테스트상품/);
...
test("inbound data viewer clipboard text includes site column before nicn", () => {
  ...
  assert.match(text, /20260324\t팬덤코리아\t테스트관리명\t테스트상품\t12/);
});
...
test("inbound data viewer applies valid site cache by exact nicn match", () => {
  ...
  assert.equal(rows[0].site, "팬덤코리아");
...
test("inbound data viewer treats empty and null-like cache entries as misses", () => {
  ...
  assert.equal(moduleUnderTest.isValidSiteValue("팬덤코리아"), true);
});
