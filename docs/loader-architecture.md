...
- `remote:status:<id>`: 스크립트별 원격 상태
- `remote:meta:<id>`: 스크립트별 최근 원격 meta
...
예전 `remote:status`, `remote:meta` blob 키가 남아 있으면 로더가 1회 lazy migration으로 `remote:*:<id>` 형태로 옮기고 legacy 키를 정리한다.
...
background refresh는 기존 정책대로 opportunistic하게 동작한다.  
반면 관리창의 수동 액션은 lock이 잡혀 있을 때 잠깐 대기한 뒤 실제 원격 확인이 끝난 경우에만 완료로 처리한다.
...
- `새로고침`: registry 강제 갱신 후 전체 원격 meta를 다시 읽어 관리창 상태를 확정한다.
- `동기화`: 대상 스크립트의 원격 meta, code, dependency를 갱신하고 현재 페이지 적용 대상이면 즉시 다시 실행한다.
- `캐시삭제`: 대상 캐시와 원격 상태를 함께 정리한다.

팝업 창을 재사용해도 이벤트 핸들러는 매번 다시 바인딩되고, 전체 작업은 전역 busy, 행 작업은 row-level busy로 표시된다.

## 원격 상태 규칙

원격 상태는 `버전 + checksum` 기준으로 계산한다.

- `new`: 로컬 캐시가 없고 원격 meta만 있는 경우
- `update`: 원격 version이 더 높은 경우
- `redeploy`: version은 같지만 checksum이 달라 같은 버전 재배포가 감지된 경우
- `clean`: 원격과 로컬이 일치하는 경우
- `error`: 수동 동기화 후에도 정합성을 확인하지 못한 경우

수동 동기화가 끝나면 대상 스크립트는 `clean` 또는 `error`로 다시 확정한다.  
따라서 stale 상태가 성공처럼 남지 않도록 유지한다.
